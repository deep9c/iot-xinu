	...
	)
{
	...
	char *udataptr = (char *)pktptr->net_udpdata;
	//kprintf("In udp_in data ..%s\n",udataptr);
	if(strstr(udataptr,"edge")!= NULL){
	kprintf("In udp_in data ..%s\n",udataptr);
		readedgedata(udataptr);
	}
	//kprintf("After readegdedata in udp_in \n");
	freebuf((char *) pktptr);
	restore(mask);
	return;
}
